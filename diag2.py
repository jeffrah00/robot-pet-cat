...
print(f"physics_dt={cat.cfg.physics_dt}")
dt=0.05
n_substeps=max(1,int(round(dt/cat.cfg.physics_dt)))
print(f"n_substeps per step={n_substeps}")
print(f"GET_UP_SETTLE_STEPS=25, so policy called every {25/n_substeps:.2f} cat.step() calls")
...
mh=float(mj_data.qpos[2])
for tick in range(300):
    cat.step(mj_data,cmd,dt)
    h=float(mj_data.qpos[2])
    if h>mh:mh=h
    if tick%50==0:print(f"tick={tick} h={h:.4f}m max={mh:.4f}m counter={cat._get_up_substep_counter}")
print(f"FINAL max_h={mh:.4f}m")
