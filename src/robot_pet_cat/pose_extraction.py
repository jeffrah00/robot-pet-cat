...
This bridges the gap between "real cat .mp4" and our 12-keypoint JSON schema
that the retargeting module consumes. The user runs this once per clip, then
runs `rpc retarget` on the resulting JSONs.

Why DeepLabCut SuperAnimal-Quadruped specifically:
  - Trained on 40K+ images of four-legged animals including cats.
  - Out-of-the-box inference — no per-clip training.
  - Hosted on Hugging Face (`mwmathis/DeepLabCutModelZoo-SuperAnimal-Quadruped`).

Alternatives if DLC is too heavy to install on a given box:
  - OpenPifPaf with the animalpose plugin (`pip install openpifpaf`).
  - MMPose AnimalPose.
  - Manual annotation via makesense.ai.
The retargeting module doesn't care which detector you used — it only sees the
JSON output.
"""
...
# Mapping: our keypoint name -> SuperAnimal-Quadruped DLC bodypart name(s).
# Lists of candidates are tried in order; the first one present in the DLC
# output wins. This lets us tolerate small naming drift between DLC versions.
#
# DLC's SuperAnimal-Quadruped bodyparts list (39 total) includes:
#   nose, upper_jaw, lower_jaw, mouth_end_right, mouth_end_left,
#   right_eye, right_earbase, right_earend, ..., left_eye, left_earbase, ...,
...
SUPERANIMAL_KP_MAP: dict[str, list[str]] = {
    ...
    # DLC calls the front "knee" what anatomists call the elbow.
...
class ExtractConfig:
    ...
    target_fps: float | None = None   # downsample frames before inference (None = source fps)
...
def run_superanimal(cfg: ExtractConfig):
    ...
    Returns the DLC output dataframe (multi-indexed by bodypart x coord).
    """
    import deeplabcut  # noqa: PLC0415  (lazy)
    ...
    # DLC writes a .h5 next to the video with a suffix indicating model.
    # Pattern: <stem>_<model_signature>.h5
    candidates = list(cfg.video_path.parent.glob(f"{cfg.video_path.stem}*.h5"))
    if not candidates:
        raise RuntimeError(
            f"DLC produced no .h5 next to {cfg.video_path}; check the run log."
        )
...
def _resolve_dlc_name(df_columns_level: list[str], candidates: list[str]) -> str | None:
    """First candidate present in the dataframe's bodypart level. None if none match."""
    available = set(df_columns_level)
    for c in candidates:
        if c in available:
            return c
    return None
...
def dlc_to_project_keypoints(
    df,
    pcutoff: float,
) -> tuple[np.ndarray, list[str]]:
    """Convert a DLC SuperAnimal dataframe to our (T, 12, 2) keypoint array.

    Low-confidence detections are replaced by NaN; downstream callers can
    interpolate or drop frames.
    """
    # DLC dataframe has multi-index columns: (scorer, bodypart, coord).
    # Drop the scorer level so we can address by (bodypart, coord).
    df = df.copy()
    ...
    for our_name in PROJECT_KP_NAMES:
        ...
        if dlc_name is None:
            raise KeyError(
                f"None of the DLC bodyparts {SUPERANIMAL_KP_MAP[our_name]!r} "
                f"are present in the model output. Available: {bodyparts}"
            )
...
def fill_missing_linear(kps: np.ndarray) -> np.ndarray:
    """Linearly interpolate over NaN frames per keypoint per coord.

    Edge NaNs are filled with nearest-neighbour. If a keypoint is NaN
    everywhere, it's left as NaN.
    """
    out = kps.copy()
    T, N, _ = out.shape
    for i in range(N):
...
def infer_floor_y_px(kps: np.ndarray) -> float:
    """Floor y in image space = the maximum paw y observed in the clip.

    Paws are indices 4..7 in our convention; max image y == lowest in image == floor.
    """
    paw_y = kps[:, 4:8, 1]
...
def infer_scale_m_per_px(kps: np.ndarray, body_length_m: float) -> float:
    ...
    return body_length_m / float(np.median(d_px))
...
def write_project_json(
    ...
    """Write keypoints to our project JSON schema."""
...
def extract(cfg: ExtractConfig) -> Path:
    """End-to-end: video -> SuperAnimal -> our JSON keypoints schema."""
    ...
    # Fill NaN gaps so the retargeting math doesn't break.
