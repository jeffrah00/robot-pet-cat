#!/usr/bin/env bash
# Standalone chain for v7.2 (rev 2). Same pattern as v7_master_chain but
# scoped to a single variant.
set -u
exec > /tmp/v7_2_chain.log 2>&1
echo "[v7.2] $(date -u) start"
...
echo "[v7.2] applying patcher"
python3 "$SCRIPTS/patch_get_up_v7_2.py" || { echo "[v7.2] patcher FAILED"; exit 1; }

PRE_RUNS=$(ls -d $LOGDIR/2026-05-22_*/ 2>/dev/null | sort)

echo "[v7.2] $(date -u) starting train.py ($ITERS iters)"
python3 scripts/train.py Unitree-Go2-GetUp \
  --agent.max-iterations $ITERS \
  > /tmp/getup_${LABEL}_train.log 2>&1
RC=$?
echo "[v7.2] $(date -u) train.py exit=$RC"

POST_RUNS=$(ls -d $LOGDIR/2026-05-22_*/ 2>/dev/null | sort)
NEW_RUN=$(comm -13 <(echo "$PRE_RUNS") <(echo "$POST_RUNS") | head -1)
[ -z "$NEW_RUN" ] && { echo "[v7.2] no new run dir, abort"; exit 2; }
echo "[v7.2] run dir: $NEW_RUN"

CKPT="${NEW_RUN}model_${FINAL}.pt"
if [ ! -f "$CKPT" ]; then
  echo "[v7.2] checkpoint $CKPT missing, using last available"
  CKPT=$(ls -1v ${NEW_RUN}model_*.pt 2>/dev/null | tail -1)
  [ -z "$CKPT" ] && { echo "[v7.2] no ckpts, abort"; exit 3; }
fi

echo "[v7.2] $(date -u) rendering"
...
[ ! -f "$VID" ] && { echo "[v7.2] no mp4, abort"; kill $PPID_PLAY 2>/dev/null; exit 4; }
sleep 90
S1=$(stat -c%s "$VID" 2>/dev/null || echo 0)
sleep 15
S2=$(stat -c%s "$VID" 2>/dev/null || echo 0)
echo "[v7.2] mp4 sizes $S1 -> $S2"
...
cd $RPC && python3 scripts/sandbox_push.py "renders/get_up_${LABEL}.mp4" \
  -m "get_up v7_2 render (rev 2: stand_fully+all_feet+fell_over only)" || echo "[v7.2] push FAILED"

echo "[v7.2] $(date -u) DONE"
