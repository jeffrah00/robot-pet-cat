...
def couch_relative_pos(env, asset_cfg=None):
    from mjlab.managers.scene_entity_config import SceneEntityCfg
    if asset_cfg is None:
        asset_cfg = SceneEntityCfg("robot")
    asset = env.scene[asset_cfg.name]
    pos = asset.data.root_link_pos_w
    cx = torch.full_like(pos[:, 0], 1.0)
    cy = torch.full_like(pos[:, 1], 0.0)
    cz = torch.full_like(pos[:, 2], 0.40)
    return torch.stack([cx - pos[:, 0], cy - pos[:, 1], cz - pos[:, 2]], dim=-1)
