...
\centering
\begin{tikzpicture}[
  box/.style={draw, rounded corners=6pt, minimum width=3.2cm, minimum height=1.2cm,
              align=center, fill=gray!10},
  arr/.style={-{Latex[length=3mm]}, thick},
  darr/.style={-{Latex[length=3mm]}, thick, dashed},
  lbl/.style={font=\small, midway}
]
  \node[box] (mood) {\textbf{Mood latent}\\6-dim OU process};
  \node[box, right=2.4cm of mood] (att)  {\textbf{Attractor}\\mode policy};
  \node[box, right=2.4cm of att]  (ppo)  {\textbf{PPO Brain}\\Discrete(5)};

  \draw[arr] (mood) -- node[above,lbl]{state[0--2]} node[below,lbl]{\small mode bias} (att);
  \draw[arr] (att)  -- node[above,lbl]{skill mask} (ppo);
  \draw[darr] (mood.south) -- ++(0,-0.7) -| node[below,lbl,pos=0.25]
      {state[3--5] $\xrightarrow{\sigma(2x)}$ $(w_\text{cur},w_\text{com},w_\text{play})$}
      (ppo.south);
  \draw[arr] (ppo.east) -- ++(0.6,0) node[right]{\small skill ID};
\end{tikzpicture}
\caption{Information flow through the three behavioral components. Mood dims
0--2 bias the attractor's mode-transition softmax; dims 3--5 set
mood-modulated reward weights for the PPO via a sigmoid. The attractor
passes an action mask constraining which skills the brain may select in the
current mode.}
\label{fig:flow}
\end{figure}
...
\texttt{renders/brain/brain3d\_random\_5min.mp4}.
