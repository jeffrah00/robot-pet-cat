...
of four learned skills (Tier~2)---both of which we
...
\begin{enumerate}[leftmargin=*,nosep]
  ...
  \item \textbf{(substrate)} An empirical demonstration that a
        \emph{small} canonical skill set (four learned) is
...
\texttt{get\_up}   & \texttt{src/.../scripted\_get\_up.py}         & Scripted PD keyframe (auto-triggered on fall; not a brain action) \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{table}[h]
  \centering\small
  \caption{10-minute autonomous rollout from
  \texttt{brain\_4skill\_v1.zip}. All six attractor modes are visited; total
  mode time sums to 600\,s. Mean dwell-times cluster just above
  ...
  Reproduce with
  \texttt{python scripts/measure\_rollout.py --duration 600}.}
  \label{tab:rollout}
  \begin{tabular}{lrccl}
    \toprule
